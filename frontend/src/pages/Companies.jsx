    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">{companies.length} records</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              ...
              <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">Website</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {companies.length === 0 && (
              <tr><td colSpan={6} className="px-6 py-12 text-center text-gray-400">No companies found</td></tr>
            )}
            {companies.map((c) => (
              <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-6 py-3.5 font-medium text-gray-900">{c.name}</td>
                <td className="px-6 py-3.5 text-gray-600">{c.industry || '—'}</td>
                <td className="px-6 py-3.5 text-gray-600">
                  {c.website
                    ? <a href={c.website} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-indigo-600 hover:underline"><Globe size={13} />{c.website.replace(/^https?:\/\//, '')}</a>
                    : '—'}
                </td>
                <td className="px-6 py-3.5 text-gray-600">
                ...
                <td className="px-6 py-3.5">
                  <div className="flex items-center gap-1 justify-end">
                    <button onClick={() => openEdit(c)} className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg"><Pencil size={14} /></button>
                    <button onClick={() => setDeleteId(c.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg"><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {deleteId && (
        <Modal title="Delete Company" onClose={() => setDeleteId(null)} size="sm">
          <p className="text-gray-600 text-sm mb-6">This will permanently delete the company.</p>
