    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">{contacts.length} records</p>
        </div>
            ...
            {contacts.map((c) => (
              <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-6 py-3.5 font-medium text-gray-900">
                  ...
                  {c.title && <span className="block text-xs text-gray-400 font-normal">{c.title}</span>}
                </td>
                <td className="px-6 py-3.5 text-gray-600">
                  {c.email ? <a href={`mailto:${c.email}`} className="flex items-center gap-1 hover:text-indigo-600"><Mail size={13} />{c.email}</a> : '—'}
                </td>
                <td className="px-6 py-3.5 text-gray-600">
                ...
                <td className="px-6 py-3.5">
                  <span className={`text-xs px-2.5 py-1 rounded-full font-medium capitalize ${STATUS_COLORS[c.status] || 'bg-gray-100 text-gray-600'}`}>{c.status}</span>
                </td>
                <td className="px-6 py-3.5">
      ...
      {deleteId && (
        <Modal title="Delete Contact" onClose={() => setDeleteId(null)} size="sm">
          <p className="text-gray-600 text-sm mb-6">This will permanently delete the contact and all related data.</p>
