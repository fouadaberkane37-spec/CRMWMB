...
const STAGES = [
  { key: 'lead', label: 'Lead', color: 'bg-blue-50 border-blue-200', badge: 'bg-blue-100 text-blue-700' },
  { key: 'qualified', label: 'Qualified', color: 'bg-indigo-50 border-indigo-200', badge: 'bg-indigo-100 text-indigo-700' },
  { key: 'proposal', label: 'Proposal', color: 'bg-purple-50 border-purple-200', badge: 'bg-purple-100 text-purple-700' },
  { key: 'negotiation', label: 'Negotiation', color: 'bg-orange-50 border-orange-200', badge: 'bg-orange-100 text-orange-700' },
  { key: 'won', label: 'Won', color: 'bg-emerald-50 border-emerald-200', badge: 'bg-emerald-100 text-emerald-700' },
  { key: 'lost', label: 'Lost', color: 'bg-red-50 border-red-200', badge: 'bg-red-100 text-red-700' },
]
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">{deals.length} deals</p>
        </div>
          ...
          return (
            <div key={stage.key} className="flex-shrink-0 w-64">
              ...
              <div className={`rounded-t-xl border-t border-x px-4 py-3 ${stage.color}`}>
                ...
                {total > 0 && <p className="text-xs text-gray-500 mt-1">{fmt(total)}</p>}
              </div>
              ...
              <div className={`border rounded-b-xl border-t-0 min-h-[200px] p-2 space-y-2 ${stage.color}`}>
                {cards.map((deal) => {
                  ...
                  return (
                    <div key={deal.id} className="bg-white rounded-lg border border-gray-100 p-3 shadow-sm">
                      <p className="text-sm font-semibold text-gray-900 leading-tight">{deal.title}</p>
                      {deal.value > 0 && (
                        <p className="text-sm text-indigo-600 font-medium mt-1 flex items-center gap-0.5">
                      ...
                      <div className="flex items-center gap-1 mt-2 pt-2 border-t border-gray-50">
                        <button
                          ...
                          disabled={!canLeft}
                          className="p-1 text-gray-300 hover:text-gray-600 disabled:opacity-20 rounded"
                          title="Move left"
                        >
                          <ChevronLeft size={14} />
                        </button>
                        ...
                        <button onClick={() => openEdit(deal)} className="p-1 text-gray-300 hover:text-gray-600 rounded"><Pencil size={13} /></button>
                        <button onClick={() => setDeleteId(deal.id)} className="p-1 text-gray-300 hover:text-red-500 rounded"><Trash2 size={13} /></button>
                      </div>
                    </div>
                  )
                })}
      ...
      {deleteId && (
        <Modal title="Delete Deal" onClose={() => setDeleteId(null)} size="sm">
          <p className="text-gray-600 text-sm mb-6">Permanently delete this deal?</p>
